<%@page import="com.mint.project.dtos.StarpointDto"%>
...
var starRating = function(){
  ...
  $(document)
    ...
    if($checked.length === 0){
      $result.text("0");sfjladkfjlasdfklawialsdfalskerf
    } else {
...
<input type="hidden" class="star_useq" value="2">
<input type="hidden" class="star_sseq" value="<%=sdto.getSseq()%>">
</div>
<div class="movieinfo">
<h3>${mdto.mtitle}</h3>
...
<img src="resources/img/M_urlcopy.png" src="url복사" onclick="w_url()">
...
<h3>별점주기</h3>
				...
				<%
					if(sdto==null){
						%>
							<input type="submit" value="별점주기" onclick="star_insert()">
						<%
					} else {
				%>
				<input type="submit" value="별점수정" onclick="star_update()">
				<%
					}
				%>
...
<div class="review_box">
	<form action="insertReview.do" method="post">
		<input type="hidden" name="ruseq" value="2">
		<input type="hidden" name="rmseq" value="${mdto.mseq}">
		<h3>한줄평 작성하기</h3>
			<table border="1">
				<col width="50"><col width="80">
				<tr>
					<td><input type="hidden" name="runick" value="토끼">아이디(로그인정보에서 가져올것. 옆에 값도!!)</td>
...
<h3>한줄평 보기</h3><h5><a href="movie_review.do?mseq=${mdto.mseq}&useq=2">모두 보기</a></h5>
<%
	int a=mdto.getMseq();
   ...
   <tr>
      ...
      <td>
         ...
         if(str.contains(target)) {
            ...
            %>
            <img src="resources/img/M_b2.png" onclick="location.href='updateDown.do?rseq=<%=rdto.getRseq()%>&useq=2&mseq=<%=a%>'">
            <%
         } else {
            System.out.println("문자열없음/아직 공감누르기 전인 리뷰");
            %>
            <img src="resources/img/M_b1.png" onclick="location.href='updateUp.do?rseq=<%=rdto.getRseq()%>&useq=2&mseq=<%=a%>'">
            <%
         }
